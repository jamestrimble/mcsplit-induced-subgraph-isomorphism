        ...
        vector<BdIt> & split_bds, BDLL & bdll, int w)
{
    ...
    for (BdIt bd_it=bdll.begin(); bd_it!=bdll.end(); bd_it=bd_it->next) {
        It l_mid = bd_it->undergoing_split ? bd_it->l_mid : bd_it->l_end;
        int l0_size = l_mid - bd_it->l;
        int r0_size = bitset_popcount_of_difference(bd_it->right_set, workspace.g1_adj_bitsets[w]);
        if (l0_size > r0_size) {
            return false;
        }
        if (!bd_it->undergoing_split && r0_size > 0) {
            bd_it->l_mid = bd_it->l_end;
        ...
        vector<Ptrs> & left_ptrs, vector<Ptrs> & right_ptrs, int w)
{
    ...
    for (auto bd_it : split_bds) {
        ...
        bitset_remove_all(bd_it->right_set, workspace.g1_adj_bitsets[w]);
        ...
        const Graph & g1)
{
    ...
    for (NewBidomain *p=split_bds_list; p!=nullptr; ) {
        ...
        bitset_add_all(bd_it->right_set, nxt.right_set);
        ...
        long long & solution_count)
{
    ...
    for (int w : ww) {
        ...
        assign(v, w, bd_it, left_ptrs, right_ptrs);
