...
int bitset_add_all_and_return_popcount(vector<unsigned long long> & bitset1,
                                     const vector<unsigned long long> & bitset2)
{
    int popcount = 0;
    for (unsigned i=0; i<bitset1.size(); i++) {
        bitset1[i] |= bitset2[i];
        popcount += __builtin_popcountll(bitset1[i]);
    }
    return popcount;
}

void bitset_add_all_of_intersection(vector<unsigned long long> & bitset1,
                                     const vector<unsigned long long> & bitset2,
                                     const vector<unsigned long long> & bitset3)
{
    for (unsigned i=0; i<bitset1.size(); i++)
        bitset1[i] |= (bitset2[i] & bitset3[i]);
}
...
template<typename F>
void bitset_foreach_in_intersection(
        const vector<unsigned long long> & bitset1,
        const vector<unsigned long long> & bitset2,
        F f)
{
        for (unsigned i=0; i<bitset1.size(); i++) {
            unsigned long long word = bitset1[i] & bitset2[i];
            while (word) {
                int bit = __builtin_ctzll(word);
                word ^= (1ull << bit);
                int v = i*BITS_PER_WORD + bit;
...
struct NewBidomain {
    vector<unsigned long long> left_set;
    vector<unsigned long long> right_set;
    union {
        NewBidomain *next_in_split_list = nullptr;
        NewBidomain *next_in_free_list;
    };
    NewBidomain *next_in_deleted_list = nullptr;
    NewBidomain *prev = nullptr;
    NewBidomain *next = nullptr;
    bool active = false;
    int _first_in_left_set = -1;
    int _l_size = 0;
    int _r_size = 0;
    long tmp;

    void resize_left_set(int sz) { left_set.resize(sz); }
    void resize_right_set(int sz) { right_set.resize(sz); }

    void initialise(It l, It r, It l_end, It r_end)
    {
        this->active = true;
        clear_bitset(left_set);
        for (auto it=l; it!=l_end; it++)
            set_bit(left_set, *it);
        clear_bitset(right_set);
        for (auto it=r; it!=r_end; it++)
            set_bit(right_set, *it);
        _first_in_left_set = first_set_bit(left_set);
        _l_size = bitset_popcount(left_set);
        _r_size = bitset_popcount(right_set);
    }

    void initialise(
            vector<unsigned long long> & l_set1,
            vector<unsigned long long> & l_set2,
            vector<unsigned long long> & r_set1,
            vector<unsigned long long> & r_set2)
    {
        this->active = true;
        bitset_intersection(l_set1, l_set2, left_set);
    ...
    void add_vtx_to_left_set(int v) {
        if (!test_bit(left_set, v)) ++_l_size;
        set_bit(left_set, v);
        _first_in_left_set = first_set_bit(left_set);
    }
    void remove_vtx_from_left_set(int v) {
        if (test_bit(left_set, v)) --_l_size;
        unset_bit(left_set, v);
        _first_in_left_set = first_set_bit(left_set);
    }
    void add_to_left_set(vector<unsigned long long> & vv) {
        bitset_add_all(left_set, vv);
        _l_size = bitset_popcount(left_set);
        _first_in_left_set = first_set_bit(left_set);
    }
    void remove_from_left_set(vector<unsigned long long> & vv) {
        bitset_remove_all(left_set, vv);
        _l_size = bitset_popcount(left_set);
        _first_in_left_set = first_set_bit(left_set);
    }
    void add_vtx_to_right_set(int w) {
        if (!test_bit(right_set, w)) ++_r_size;
        set_bit(right_set, w);
    }
    void remove_vtx_from_right_set(int w) {
        if (test_bit(right_set, w)) --_r_size;
        unset_bit(right_set, w);
    }

    int first_in_left_set() const { return _first_in_left_set; }
    int l_size() const { return _l_size; }
    int r_size() const { return _r_size; }
};
...
struct BitGraphPair {
    vector<vector<unsigned long long>> g0_adj_bitsets;
    vector<vector<unsigned long long>> g1_adj_bitsets;
    BitGraphPair(Graph & g0, Graph & g1) :
            g0_adj_bitsets(g0.n, vector<unsigned long long>(g0.bitset_word_count())),
            g1_adj_bitsets(g1.n, vector<unsigned long long>(g1.bitset_word_count())) {}
};
...
struct Workspace {
    ...
    struct Graph & g0;
    struct Graph & g1;
    vector<BitGraphPair> bit_graphs;

    // for each g0 vertex v, we have a bitset of which g1 vertices v may be mapped
    // to according to the vertices' degrees
    vector<vector<unsigned long long>> possible_g1_vertices;

    // for all-diff
    vector<BdIt> sorted_bd_its;
    vector<unsigned long long> l_bitset;
    vector<unsigned long long> r_bitset;

    NewBidomain *get_from_free_list()
    {
        if (bd_free_list == nullptr) {
            bd_memory_pools.push_back(vector<NewBidomain>(100));
            int l_bitset_sz = g0.bitset_word_count();
            int r_bitset_sz = g1.bitset_word_count();
            for (NewBidomain & bd : bd_memory_pools.back()) {
                bd.next_in_free_list = bd_free_list;
                bd_free_list = &bd;
                bd.resize_left_set(l_bitset_sz);
                bd.resize_right_set(r_bitset_sz);
            }
        }
    ...
    void make_supplementals()
    {
        for (int v=0; v<g0.n; v++)
            for (int w : g0.filtered_adj_lists[v])
                set_bit(bit_graphs[0].g0_adj_bitsets[v], w);
        for (int v=0; v<g1.n; v++)
            for (int w : g1.filtered_adj_lists[v])
                set_bit(bit_graphs[0].g1_adj_bitsets[v], w);
//         for (int v=0; v<g0.n; v++) {
//             for (int w : g0.filtered_adj_lists[v]) {
// //                bitset_add_all_of_intersection(bit_graphs[3].g0_adj_bitsets[v],
// //                        bit_graphs[0].g0_adj_bitsets[w],
// //                        bit_graphs[2].g0_adj_bitsets[v]);
// //                bitset_add_all_of_intersection(bit_graphs[2].g0_adj_bitsets[v],
// //                        bit_graphs[0].g0_adj_bitsets[w],
// //                        bit_graphs[1].g0_adj_bitsets[v]);
//                 bitset_add_all(bit_graphs[1].g0_adj_bitsets[v], bit_graphs[0].g0_adj_bitsets[w]);
//             }
//             unset_bit(bit_graphs[1].g0_adj_bitsets[v], v);
// //            unset_bit(bit_graphs[2].g0_adj_bitsets[v], v);
// //            unset_bit(bit_graphs[3].g0_adj_bitsets[v], v);
//         }
//         for (int v=0; v<g1.n; v++) {
//             for (int w : g1.filtered_adj_lists[v]) {
// //                bitset_add_all_of_intersection(bit_graphs[3].g1_adj_bitsets[v],
// //                        bit_graphs[0].g1_adj_bitsets[w],
// //                        bit_graphs[2].g1_adj_bitsets[v]);
// //                bitset_add_all_of_intersection(bit_graphs[2].g1_adj_bitsets[v],
// //                        bit_graphs[0].g1_adj_bitsets[w],
// //                        bit_graphs[1].g1_adj_bitsets[v]);
//                 bitset_add_all(bit_graphs[1].g1_adj_bitsets[v], bit_graphs[0].g1_adj_bitsets[w]);
//             }
//             unset_bit(bit_graphs[1].g1_adj_bitsets[v], v);
// //            unset_bit(bit_graphs[2].g1_adj_bitsets[v], v);
// //            unset_bit(bit_graphs[3].g1_adj_bitsets[v], v);
//         }
    }

    Workspace(struct Graph & g0, struct Graph & g1)
            : g0(g0),
            g1(g1),
            bit_graphs(1 /*4*/, {g0, g1}),
            possible_g1_vertices(g0.n),
            l_bitset(g0.bitset_word_count()),
            r_bitset(g1.bitset_word_count())
    {
    }
private:
...
void show(const Graph & g0, const Graph & g1, const vector<VtxPair>& current, BDLL & bdll)
{
    ...
    for (BdIt bd_it=bdll.begin(); bd_it!=bdll.end(); bd_it=bd_it->next) {
        cout << "Left  ";
        bitset_foreach(bd_it->left_set, [](int v) { cout << v << " "; });
        cout << std::endl;
        cout << "Right  ";
...
BdIt select_bidomain_heur_A(Workspace & workspace, BDLL & domains)
{
    ...
    for (BdIt bd_it=domains.begin(); bd_it!=domains.end(); bd_it=bd_it->next) {
        auto const & bd = *bd_it;
        int tie_breaker = bd.first_in_left_set();
        int right_len = bitset_popcount_of_intersection(bd.right_set,
                workspace.possible_g1_vertices[tie_breaker]);//bd.r_size();
...
BdIt select_bidomain_heur_B(Workspace & workspace, BDLL & domains, const Graph & g0)
{
    double best_score = INT_MIN;
    BdIt best = domains.end();
    for (BdIt bd_it=domains.begin(); bd_it!=domains.end(); bd_it=bd_it->next) {
        auto const & bd = *bd_it;
        int v = bd.first_in_left_set();
        int right_len = bitset_popcount_of_intersection(bd.right_set,
                workspace.possible_g1_vertices[v]);//bd.r_size();
        //int right_len = bd.r_size();
        if (right_len <= 1) {
            // Special case where no branching is required
            return bd_it;
        }
        int deg = g0.adj_lists[v].size();
...
BdIt select_bidomain(Workspace & workspace, BDLL & domains, const Graph & g0)
{
    if (arguments.heuristic == heur_A)
        return select_bidomain_heur_A(workspace, domains);
    else
        return select_bidomain_heur_B(workspace, domains, g0);
}
...
bool partition(Workspace & workspace,
        vector<BdIt> & split_bds, BDLL & bdll, int v, int w, BitGraphPair & bit_graph_pair)
{
    for (BdIt bd_it=bdll.begin(); bd_it!=bdll.end(); bd_it=bd_it->next) {
        int l1_size = bitset_popcount_of_intersection(bd_it->left_set, bit_graph_pair.g0_adj_bitsets[v]);
        if (l1_size) {
            int r1_size = bitset_popcount_of_intersection(bd_it->right_set, bit_graph_pair.g1_adj_bitsets[w]);
            if (l1_size > r1_size) {
                return false;
            }
...
NewBidomain * do_splits(Workspace & workspace, vector<BdIt> & split_bds, int v, int w,
        BitGraphPair & bit_graph_pair)
{
    ...
    for (auto bd_it : split_bds) {
        ...
        new_elem->initialise(bd_it->left_set, bit_graph_pair.g0_adj_bitsets[v],
                bd_it->right_set, bit_graph_pair.g1_adj_bitsets[w]);
        ...
        bd_it->remove_from_left_set(bit_graph_pair.g0_adj_bitsets[v]);
    }
...
NewBidomain * do_deletions(vector<BdIt> & split_bds)
{
    ...
    for (auto bd_it : split_bds) {
        // Delete old BD if left set is empty
        auto & bd = *bd_it;
        if (bd.l_size() == 0) {
            bd.active = false;
...
SplitAndDeletedLists filter_domains(
        Workspace & workspace, BDLL & bdll, int v, int w, BitGraphPair & bit_graph_pair)
{
    vector<BdIt> & split_bds = workspace.split_bds;
    split_bds.clear();

    bool partition_success = partition(workspace, split_bds, bdll, v, w, bit_graph_pair);
    ...
    NewBidomain *split_bds_list = do_splits(workspace, split_bds, v, w, bit_graph_pair);
        ...
        NewBidomain *deleted_bds)
{
    ...
    for (NewBidomain *p=split_bds_list; p!=nullptr; ) {
        ...
        bd_it->add_to_left_set(nxt.left_set);
...
bool assign(int v, int w, BdIt bd_it, BDLL & bdll,
        vector<BdIt> & bds_with_w_removed /* filled in by this function */)
{
    bd_it->remove_vtx_from_left_set(v);

    for (BdIt bd_it=bdll.begin(); bd_it!=bdll.end(); bd_it=bd_it->next) {
        if (test_bit(bd_it->right_set, w)) {
            if (bd_it->r_size() <= bd_it->l_size()) {
                return false;
            }
            bds_with_w_removed.push_back(bd_it);
            bd_it->remove_vtx_from_right_set(w);
        }
    }
    return true;
}

void unassign(int v, int w, BdIt bd_it, vector<BdIt> & bds_with_w_removed)
{
    bd_it->add_vtx_to_left_set(v);

    for (BdIt bd_it : bds_with_w_removed)
        bd_it->add_vtx_to_right_set(w);
}

bool cheap_all_diff_fails(Workspace & workspace, const Graph & g0, const Graph & g1, BDLL & bdll) {
    vector<BdIt> & sorted_bd_its = workspace.sorted_bd_its;
    sorted_bd_its.clear();
    for (BdIt bd_it=bdll.begin(); bd_it!=bdll.end(); bd_it=bd_it->next) {
        bd_it->tmp = bd_it->r_size();// * 1000000 - bitset_popcount(bd_it->left_set);
        sorted_bd_its.push_back(bd_it);
    }
    std::sort(sorted_bd_its.begin(), sorted_bd_its.end(),
            [&](BdIt a, BdIt b){ return a->tmp < b->tmp; });

    vector<unsigned long long> & l_bitset = workspace.l_bitset;
    vector<unsigned long long> & r_bitset = workspace.r_bitset;
    clear_bitset(l_bitset);
    clear_bitset(r_bitset);

    for (BdIt bd_it : sorted_bd_its) {
        int l_pc = bitset_add_all_and_return_popcount(l_bitset, bd_it->left_set);
        int r_pc = bitset_add_all_and_return_popcount(r_bitset, bd_it->right_set);
        //int l_pc = bitset_popcount(l_bitset);
        //int r_pc = bitset_popcount(r_bitset);
        if (r_pc >= g0.n) return false;  // an optimisation
        if (l_pc > r_pc) return true;
    }
    return false;
}

void solve(Workspace & workspace, const Graph & g0, const Graph & g1, vector<VtxPair> & incumbent,
        vector<VtxPair> & current, BDLL & bdll, long long & solution_count)
{
    if (abort_due_to_timeout)
        return;

    if (arguments.verbose) show(g0, g1, current, bdll);
    nodes++;
    ...
    if (cheap_all_diff_fails(workspace, g0, g1, bdll))
        return;

    BdIt bd_it = select_bidomain(workspace, bdll, g0);
        
    if (bd_it == bdll.end())
        return;

    int v = bd_it->first_in_left_set();

    vector<unsigned long long> ww_bitset(bd_it->right_set);
    std::vector<int> ww;
    ww.reserve(bitset_popcount(ww_bitset));
    bitset_foreach_in_intersection(ww_bitset, 
            workspace.possible_g1_vertices[v], [&ww](int w){ ww.push_back(w); });

    vector<BdIt> bds_with_w_removed;
    vector<SplitAndDeletedLists> filter_results;

    // Try assigning v to each vertex w in the colour class beginning at bd.r, in turn
    for (int w : ww) {
        bds_with_w_removed.clear();
        if (assign(v, w, bd_it, bdll, bds_with_w_removed)) {
            ...
            for (unsigned i=0; i<workspace.bit_graphs.size(); i++) {
                auto filter_result = filter_domains(workspace, bdll, v, w, workspace.bit_graphs[i]);
                filter_results.push_back(filter_result);
                if (filter_results.back().quit_early) break;
            }

            // TODO: tidy up all this checking for quitting early somehow?
            if (!filter_results.back().quit_early) {
                current.push_back(VtxPair(v, w));

                solve(workspace, g0, g1, incumbent, current, bdll, solution_count);

                current.pop_back();
            }
            for (unsigned i=filter_results.size(); i--; ) {
                auto filter_result = filter_results[i];
                if (filter_result.quit_early) continue;
                unfilter_domains(workspace, bdll, filter_result.split_bds_list, filter_result.deleted_bds_list);
            }
        ...
        unassign(v, w, bd_it, bds_with_w_removed);
...
std::pair<vector<VtxPair>, long long> mcs(Graph & g0, Graph & g1)
{
    ...
    for (unsigned int label : labels) {
        ...
        NewBidomain *new_elem = nullptr;

        It r_hi = new_right.begin() + start_r;
        It prev_r_hi;
        vector<unsigned long long> possible_g1_vertices(g1.bitset_word_count());
        for (It v_it = new_left.begin() + start_l; v_it != new_left.begin() + start_l + left_len; v_it++) {
            int v = *v_it;
            unsigned v_deg = g0.adj_lists[v].size();
            while (r_hi != new_right.begin() + start_r + right_len && g1.adj_lists[*r_hi].size() >= v_deg) {
                set_bit(possible_g1_vertices, *r_hi);
                ++r_hi;
            }

            if (v_it - (new_left.begin() + start_l) > r_hi - (new_right.begin() - start_r)) {
                // Even if this was removed, the cheap alldiff would probably do most
                // of the same job.
                return {{}, 0};
            }

            if (v_it == new_left.begin() + start_l || r_hi != prev_r_hi) {
                prev_r_hi = r_hi;
                new_elem = workspace.get_from_free_list();
                new_elem->insert_before(&bdll.head);
                new_elem->initialise(v_it,
                                  new_right.begin() + start_r,
                                  v_it + 1,
                                  r_hi);
            } else {
                new_elem->add_vtx_to_left_set(v);
                //set_bit(new_elem->left_set, v);
            }
            workspace.possible_g1_vertices[v] = possible_g1_vertices;
        }
        //vector<unsigned long long> possible_g1_vertices(g1.bitset_word_count());
        //for (It v_it = new_left.begin() + start_l; v_it != new_left.begin() + start_l + left_len; v_it++) {
        //    int v = *v_it;
